...
\begin{example}[Linguaggio $L = \{a^n b^n \mid n \ge 0\}$]
    Questo linguaggio, che rappresenta stringhe con $n$ 'a' seguite da $n$ 'b' (es. `aabb`, `aaabbb`), \textbf{non può essere riconosciuto da un automa a stati finiti}.
    \begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Simbolo Blank ($\texttt{B}$)}: Le celle vuote del nastro contengono un simbolo speciale (blank) per delimitare la stringa di input.
...
\begin{figure}[h!]
    ...
    \node[head] at (0,1.2) {$\delta$};
    \draw[->] (head) -- (0,0.35);
    \node[below=0.5cm of tape] (blank_sym) {$\texttt{B}$ (Blank)};
    \end{tikzpicture}
...
1.  \textbf{$q_0$ (Stato Iniziale)}:
    \begin{itemize}
        \item Se legge \texttt{A}: Scrive \texttt{B} (blank), sposta a destra ($\texttt{R}$), va in $q_1$. (Cancella la prima 'a').
        \item Se legge \texttt{B} o \texttt{B} (blank, dopo aver cancellato tutto): Significa che non ci sono più 'a' da cancellare, quindi verifica se anche le 'b' sono finite. Se legge \texttt{B} e sposta a destra, va in $q_4$ (stato accettante).
    \end{itemize}
2.  \textbf{$q_1$ (Cerca l'ultima 'b')}:
    \begin{itemize}
        ...
        \item Se legge \texttt{B}: Scrive \texttt{B}, sposta a destra ($\texttt{R}$), rimane in $q_1$. (Salta le 'b' fino alla fine).
        \item Se legge \texttt{B} (blank): Scrive \texttt{B}, sposta a sinistra ($\texttt{L}$), va in $q_2$. (Ha raggiunto la fine della stringa, torna indietro per trovare l'ultima 'b').
    \end{itemize}
3.  \textbf{$q_2$ (Cancella l'ultima 'b')}:
    \begin{itemize}
        \item Se legge \texttt{B}: Scrive \texttt{B} (blank), sposta a sinistra ($\texttt{L}$), va in $q_3$. (Cancella l'ultima 'b' e inizia a tornare all'inizio della stringa).
    \end{itemize}
4.  \textbf{$q_3$ (Torna all'inizio)}:
    \begin{itemize}
        ...
        \item Se legge \texttt{B}: Scrive \texttt{B}, sposta a sinistra ($\texttt{L}$), rimane in $q_3$. (Salta le 'b' rimanenti, andando a sinistra).
        \item Se legge \texttt{B} (blank): Scrive \texttt{B}, sposta a destra ($\texttt{R}$), va in $q_0$. (Ha raggiunto l'inizio della stringa, torna a $q_0$ per la prossima iterazione).
    \end{itemize}
...
\begin{itemize}
    \item Se $q_0$ legge \texttt{B} all'inizio, si muove a $q_4$, quindi accetterebbe la stringa vuota $\epsilon$. Ma se è $B$ e non $\epsilon$, si muoverebbe a $q_4$ e accetterebbe una stringa di soli $B$s (e.g., $B$). Questo è un dettaglio che richiede un'attenzione specifica sul caso base $\epsilon$ o stringhe di soli $B$s.
...
\begin{definition}[Configurazione di una TM]
    ...
    \begin{example}
        \begin{itemize}
            \item $A q_1 BB$: La stringa sul nastro è $\texttt{ABB}$, la TM è nello stato $q_1$, e la testina sta leggendo il primo $\texttt{B}$.
            \item $ABB q_1 \texttt{B}$: La stringa sul nastro è $\texttt{ABB}$, la TM è nello stato $q_1$, e la testina sta leggendo il primo blank a destra della stringa.
        \end{itemize}
    \end{example}
\end{definition}
