...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[shorten >=1pt,node distance=2.5cm,on grid,auto]
        ...
        \path[->]
        (q0) edge node {A/B, R} (q1) % First A to blank
        (q1) edge [loop right] node {\parbox{1.5cm}{\centering A/A, R \\ B/B, R}} (q1) % Skip A's and B's
        (q1) edge node {B/B, L} (q2) % Found Blank, go left to find last B (Correction based on lecture: should be Blank/Blank, L)
        (q2) edge [loop right] node {B/B, L} (q2) % Skip B's backwards
        (q2) edge node {A/B, L} (q3) % Found A, replace with Blank, go left (Correction based on lecture: should be B/Blank, L)
        (q3) edge [loop left] node {\parbox{1.5cm}{\centering A/A, L \\ B/B, L}} (q3) % Skip A's and B's backwards
        (q3) edge node {B/B, R} (q0) % Found Blank, go right to q0 (Correction based on lecture: should be Blank/Blank, R)
        (q0) edge node {B/B, R} (q4); % If all A's and B's are matched, hit blank and accept (Correction based on lecture: should be Blank/Blank, R)
    \end{tikzpicture}
