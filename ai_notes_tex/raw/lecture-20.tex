Ecco gli appunti in formato LaTeX basati sulla trascrizione della lezione:
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=1.5cm, thick]
        ...
        \foreach \i in {1,2,3,4} {
            \node[circle, draw, right=2cm of T] (x\i) {$x_{\i}$};
            \node[circle, draw, below=0.5cm of x\i] (nx\i) {$\neg x_{\i}$};
            \draw (x\i) -- (nx\i);
            \draw (x\i) -- (B);
            \draw (nx\i) -- (B);
            \ifnum\i<4
                \pgfmathsetmacro{\nextx}{\i+1}
                \node[right=1.5cm of x\i] (dummyx\nextx) {};
                \node[below=0.5cm of dummyx\nextx] (dummynx\nextx) {};
            \fi
        }
