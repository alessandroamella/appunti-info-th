...
\begin{tikzpicture}[scale=0.8]
    % Definizione dei colori per i lati delle piastrelle
    ...
    % Griglia di sfondo per il primo quadrante
    \draw[lightgray, very thin] (0,0) grid (5,4);
    ...
    % Esempio di piastrelle con colori sui lati
    
    % Piastrella D0 nell'origine
    \filldraw[fill=white, draw=black, line width=2pt] (0,0) rectangle (1,1);
    \draw[rosso, line width=4pt] (0,0) -- (1,0);    % lato inferiore rosso
    \draw[blu, line width=4pt] (1,0) -- (1,1);      % lato destro blu
    \draw[verde, line width=4pt] (1,1) -- (0,1);    % lato superiore verde
    \draw[giallo, line width=4pt] (0,1) -- (0,0);   % lato sinistro giallo
    \node at (0.5,0.5) {$D_0$};
    
    % Piastrella compatibile a destra
    \filldraw[fill=white, draw=black, line width=2pt] (1,0) rectangle (2,1);
    \draw[rosso, line width=4pt] (1,0) -- (2,0);    % lato inferiore rosso
    \draw[verde, line width=4pt] (2,0) -- (2,1);    % lato destro verde
    \draw[viola, line width=4pt] (2,1) -- (1,1);    % lato superiore viola
    \draw[blu, line width=4pt] (1,1) -- (1,0);      % lato sinistro blu (compatibile)
    \node at (1.5,0.5) {$D_1$};
    
    % Piastrella compatibile sopra
    \filldraw[fill=white, draw=black, line width=2pt] (0,1) rectangle (1,2);
    \draw[verde, line width=4pt] (0,1) -- (1,1);    % lato inferiore verde (compatibile)
    \draw[arancione, line width=4pt] (1,1) -- (1,2); % lato destro arancione
    \draw[giallo, line width=4pt] (1,2) -- (0,2);   % lato superiore giallo
    \draw[rosso, line width=4pt] (0,2) -- (0,1);    % lato sinistro rosso
    \node at (0.5,1.5) {$D_2$};
    
    % Piastrella compatibile in diagonale
    \filldraw[fill=white, draw=black, line width=2pt] (1,1) rectangle (2,2);
    \draw[viola, line width=4pt] (1,1) -- (2,1);    % lato inferiore viola (compatibile)
    \draw[blu, line width=4pt] (2,1) -- (2,2);      % lato destro blu
    \draw[verde, line width=4pt] (2,2) -- (1,2);    % lato superiore verde
    \draw[arancione, line width=4pt] (1,2) -- (1,1); % lato sinistro arancione (compatibile)
    \node at (1.5,1.5) {$D_3$};
    
    % Esempio di piastrelle NON compatibili (con X rossa)
    \filldraw[fill=lightgray, draw=black, line width=2pt, opacity=0.5] (2,0) rectangle (3,1);
    \draw[rosso, line width=4pt] (2,0) -- (3,0);    % lato inferiore rosso
    \draw[rosso, line width=4pt] (3,0) -- (3,1);    % lato destro rosso (NON compatibile)
    \draw[blu, line width=4pt] (3,1) -- (2,1);      % lato superiore blu
    \draw[verde, line width=4pt] (2,1) -- (2,0);    % lato sinistro verde (NON compatibile)
    \draw[red, line width=3pt] (2.2,0.2) -- (2.8,0.8);
    \draw[red, line width=3pt] (2.8,0.2) -- (2.2,0.8);
    \node at (2.5,0.5) {$\times$};
    
    % Frecce per indicare le relazioni
    \draw[->, thick, red] (0.5,-0.8) -- (0.5,-0.2);
    \node[below] at (0.5,-0.8) {\textbf{Posizione fissa}};
    
    % Legenda dei colori
    \begin{scope}[shift={(6,0)}]
        \node[above] at (1.2,3.5) {\textbf{Colori dei lati:}};
        \draw[rosso, line width=4pt] (0,3) -- (0.5,3);
        \node[right] at (0.6,3) {Rosso};
        \draw[blu, line width=4pt] (0,2.5) -- (0.5,2.5);
        \node[right] at (0.6,2.5) {Blu};
        \draw[verde, line width=4pt] (0,2) -- (0.5,2);
        \node[right] at (0.6,2) {Verde};
        \draw[giallo, line width=4pt] (0,1.5) -- (0.5,1.5);
        \node[right] at (0.6,1.5) {Giallo};
        \draw[viola, line width=4pt] (0,1) -- (0.5,1);
        \node[right] at (0.6,1) {Viola};
        \draw[arancione, line width=4pt] (0,0.5) -- (0.5,0.5);
        \node[right] at (0.6,0.5) {Arancione};
    \end{scope}
    
    % Regole di affiancamento
    \begin{scope}[shift={(11,1)}]
        \node[above] at (0,2) {\textbf{Regole:}};
        \node[align=left] at (0,1.5) {$(D_0, D_1) \in H$};
        \node[align=left] at (0,1) {$(D_0, D_2) \in V$};
        \node[align=left] at (0,0.5) {$(D_1, D_3) \in V$};
        \node[align=left] at (0,0) {$(D_2, D_3) \in H$};
    \end{scope}
    
\end{tikzpicture}
\caption{Esempio di Tiling Problem: la piastrella $D_0$ è posizionata forzatamente in $(0,0)$. Le piastrelle adiacenti devono avere colori compatibili sui lati che si toccano. La piastrella barrata in grigio mostra un posizionamento non valido.}
