...
\begin{tikzpicture}[scale=0.7, transform shape, node distance=1.5cm and 2.5cm]
    ...
    \path (q8) edge[loop above] node[edge label] {
        $\{\alphaSym;\alphaSym,R$ / $\any;\any,S$ / $\any;\any,S$ / $\any;\any,S$ / $\any;\any,S\}$, \\
        $\{\any;\any,S$ / $\any;\any,S$ / $\X;\X,L$ / $\any;\any,S$ / $\any;\any,S\}$} (q8);
    \path (q8) edge[bend left=35] node[edge label, right] {$\Sh;\Sh,S$ / $\any;\any,S$ / $\B;\B,S$ \\ / $\B;\B,S$ / $\B;\B,S$} (q1);
    
    \path (q7) edge node[edge label] {$\B;\B,S$ / $\B;\B,S$ \\ / $\dots$} (q_acc);
\end{tikzpicture}
